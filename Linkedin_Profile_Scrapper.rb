...
##<USAGE> : LinkedinSearch <CONFIG_FILE> <INPUT FILE LOCATION> <OUTPUT FILE LOCATION>
class LinkedinSearch
 ...
 def  self.init_config
  ...
  @@TOP_SKILL_WINDOW=cfg['TOP_SKILL_WINDOW'].nil? ? 10 : cfg['TOP_SKILL_WINDOW']
  @@BOTTOM_SKILL_WINDOW=cfg['BOTTOM_SKILL_WINDOW'].nil? ? 10 : cfg['BOTTOM_SKILL_WINDOW']
 ...
 if page_no==0
    ...
    data=Nokogiri::HTML(open(url))
     ...
     skills=skills.split(/\n+/).map{|elem| elem.gsub(/\s+/, "")}
      skills.each do |skill|
       if !skill.empty?
        @@USER_SKILLS[skill.to_s]= (@@USER_SKILLS[skill.to_s].to_i) + 1
       end 
      end
     count=count+1
     @@PROFILES_DONE[url]=1 
     ...
     result_url.each do |elem| 
      if elem['pagemap']  
       if elem['pagemap']['hcard']
         if elem['pagemap']['hcard'].to_s.downcase.gsub(/\s+/, "").include? keyword.downcase.gsub(/\s+/, "")
  ...
  def self.execute_call
   self.init_config   
    #puts get_json_data("http://www.linkedin.com/pub/dr-v-p-singh/15/455/61a")
